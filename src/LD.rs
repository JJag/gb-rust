...
pub fn ld_bc_nn(&mut self, nn: u16) { self.set_bc(nn) }
pub fn ld_de_nn(&mut self, nn: u16) { self.set_de(nn) }
pub fn ld_hl_nn(&mut self, nn: u16) { self.set_hl(nn) }
pub fn ld_sp_nn(&mut self, nn: u16) { self.sp = nn }


pub fn ld_sp_hl(&mut self) { self.sp = self.hl() }

pub fn push_bc(&mut self) {
   mmu::write_word(self.bc(), self.sp - 2);
   self.sp = self.sp - 2
 }

pub fn push_de(&mut self) {
    mmu::write_word(self.de(), self.sp - 2);
    self.sp = self.sp - 2
  }

pub fn push_hl(&mut self) {
     mmu::write_word(self.hl(), self.sp - 2);
     self.sp = self.sp - 2
   }
pub fn push_af(&mut self) {
      mmu::write_word(self.af(), self.sp - 2);
      self.sp = self.sp - 2
    }
