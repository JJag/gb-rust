...
pub fn build_framebuffer(mmu: &Mmu) -> Array2D {
    ...
    render_sprites(mmu, &mut out_buf);
    out_buf
}
...
fn render_sprites(mmu: &Mmu, out_buf: &mut Array2D) {
    ...
    for spr in oam.sprites.iter() {
        draw_sprite(ppu, spr, &vram.tileset, out_buf);
    }
}
...
pub fn render_framebuffer(
    ...
    let buf = build_framebuffer(mmu);
    render_buf(window, e, &buf);
}

pub fn render_tileset(window: &mut PistonWindow, e: &Event, vram: &Vram) {
    let buf = self::render_tileset_to_buf(vram);
    render_buf(window, e, &buf);
}

/// 24 rows - 16 tiles each
fn render_tileset_to_buf(vram: &Vram) -> Array2D {
    let tileset = &vram.tileset;
...
pub fn render_tilemap(
    ...
    let buf = render_tilemap_to_buf(vram, sc_x, sc_y);
    render_buf(window, e, &buf);
}

fn render_tilemap_to_buf(vram: &Vram, _sc_x: u8, _sc_y: u8) -> Array2D {
...
fn render_buf(w: &mut PistonWindow, e: &Event, buf: &Array2D) {
    use piston_window::*;
